"""Model-free language gate for active local-model requests.

The policy deliberately validates only explicit metadata and provider-facing
structure.  It does not inspect prose, guess a language from vocabulary, or
translate content.  Historical artifacts and source documents remain outside
this runtime dispatch boundary.
"""
...
_FAILURE_SUMMARIES: Final[Mapping[LocalModelLanguageFailureCode, str]] = {
    ...
    "non_english_field": (
        "The provider-facing payload contains a non-English presentation field."
    ),
    "non_english_presentation": (
...
# These are structural provider-payload fields, not words searched for in prose.
...
_PRESENTATION_MODE_KEYS: Final = frozenset(
    {
        "presentation_mode",
        "text_presentation_mode",
    }
)


class LocalModelLanguagePolicyError(ValueError):
    """Bounded, content-free rejection at the local-model dispatch boundary."""
...
def require_english_local_model_payload(
    *,
    ...
    """Fail closed unless one provider request is structurally English-only.

    ``declared_language`` is intentionally strict: aliases, case folding, and
    implicit defaults are rejected.  Payload text is opaque data; only known
    language metadata, presentation-mode metadata, and forbidden field names
    are examined recursively.
    """
