...
from app.backend.rei.diagnostics import InvariantReport
...
from app.backend.rei.models.ego import EgoMeasure
from app.backend.rei.models.instinkt import InstinktProjectionObservation
...
from app.backend.rei.models.run import NativeMindBundle, RunManifest
...
def test_native_runtime_ast_boundary_rejects_transitional_or_archive_imports() -> None:
    sources = sorted((ROOT / "app" / "backend" / "rei").rglob("*.py"))
    sources.extend((RUNNER, MATRIX_RUNNER))
    findings: list[str] = []
    forbidden_roots = {"archive", "rei" + "_next"}
    ...
    assert not findings, "Native runtime crosses the cutover boundary:\n" + "\n".join(findings)
