...
from app.backend.rei_next.ids import canonical_json_bytes
...
from app.backend.rei_next.persistence import ArtifactIntegrityError, FileArtifactStore
...
from app.gui_next import server
from app.gui_next import view_model
...
def completed_views(tmp_path_factory: pytest.TempPathFactory):
    root = tmp_path_factory.mktemp("gui-next-view")
...
def test_gui_next_routes_and_imports_exclude_old_runtime_and_dataset_actions() -> None:
    route_paths = {
    ...
    backend_files = (
        ROOT / "app" / "gui_next" / "__init__.py",
        ROOT / "app" / "gui_next" / "view_model.py",
        ROOT / "app" / "gui_next" / "server.py",
    )
    ...
    assert not any(
        module == "rei"
        or module.startswith("rei.")
        or module == "app.backend.rei"
        or module.startswith("app.backend.rei.")
        for module in imported
    )
    assert not any(
        token in module.casefold()
        for module in imported
        for token in ("dataset", "ollama")
    )

    frontend = (ROOT / "app" / "gui_next" / "static" / "app.js").read_text(
        encoding="utf-8"
    )
